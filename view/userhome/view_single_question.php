        ...
        <div class="col-md-8">
            <div class="question_body_sQ">
                <div class="single_question">
                    ...
                    <br><?php if (Session::get('user') == $allQuestion->email){?>
                                ...
                                <a href="delete_question.php?qid=<?php echo serialize(time()),$allQuestion->qid;?>&b=<?php echo time();?>"><i class="fa fa-trash"></i></a>&nbsp; Me
                            <?php }else{ echo $allQuestion->first_name;}?> &nbsp; &nbsp;<?php echo $allQuestion->question_time; ?></p>
                </div>
            </div> <br><br>
            ...
            <div class="ans">
                <div class="single_ans">
                    <p><i class="fa fa-comment"></i> <?php echo $ans->answer?></p>
                    <p><?php if (Session::get('user') == $ans->email){?>
                            <a href="edit_ans.php?qid=<?php echo serialize(time()),$allQuestion->qid;?>&ansid=<?php echo  serialize(time()),$ans->ansid;?>&b=<?php echo time();?>"><i class="fa fa-pencil-square"></i></a>
                            <a href="delete_ans.php?qid=<?php echo serialize(time()),$allQuestion->qid;?>&ansid=<?php echo serialize(time()),$ans->ansid;?>&b=<?php echo time();?>"><i class="fa fa-trash"></i></a>&nbsp; Me
                        <?php }else { echo $ans->first_name ;}?> &nbsp; &nbsp; <?php echo $ans->time; ?></p>
                </div>
            </div><br>
            <?php } ?>

            <form action="answer_store.php" method="post">
                <div class="field">
                    <p>Give your answer</p>
                    <textarea id="question" rows="7" cols="50" name="answer" placeholder="Description"></textarea><br>
                    <input type="submit" value="Submit">
                    <input type="hidden" name="qid" value="<?php echo $qid?>">
                    <input type="hidden" name="ansById" value="<?php echo $allData->id?>">
                </div>
            </form>
        </div>
